...
class nnUNetTrainer(object):
    ...
    def on_train_start(self):
        ...
        # ADDED from MR
        # get monai dataloader for training
        from meld_find.config import configurations
        from meld_find.data_loader import (load_dataset,split_dataset,Dataloader,Dataloader_randomsampling)
        ...
        train_dict_list, _ = dataset_folds[config['FOLD']]
        dataloader = Dataloader_randomsampling(config)
        self.train_loader_random, self.train_loader_foreground =  dataloader.build_loader_trainval(train_dict_list, mode='train')
        # self.val_loader_random, self.val_loader_foreground =  dataloader.build_loader_trainval(val_dict_list, mode='val')

        # add dataloader nnunet for val
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        # added to compensate the fact that data and target are metatensor instead of tensor
        target = [target_i.as_tensor() for target_i in target]
        data = data.as_tensor()
        ...
        # print(f"Time to compute loss : {time() - time_start} seconds")
            # ####################################
            # #ADDED BY MR - NEED TO REMOVE
        ...
        # print(f"Time to backpropagate : {time() - time_start} seconds")
        return {
    ...
    def run_training(self):
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            # ADDED by MR: to run monai training loader
            for batch_id, (batch_random, batch_foreground) in enumerate(zip(self.train_loader_random, self.train_loader_foreground)):
                if batch_id >= self.num_iterations_per_epoch:
                    break
            ...
            with torch.no_grad():
                ...
                val_outputs = []
                # #added by MR
                # for batch_id, (batch_random, batch_foreground) in enumerate(zip(self.val_loader_random, self.val_loader_foreground)):
                #     if batch_id >= self.num_val_iterations_per_epoch:
                #         break
                #     data_val = torch.cat([batch_random["data"], batch_foreground["data"]], dim=0, ).to(self.device)
                #     target_val= torch.cat([batch_random["target"], batch_foreground["target"]], dim=0)
                #     target_val_ds = get_downsampled_labels_ds(target_val, 4, self.configuration_manager.network_arch_init_kwargs['strides'])
                #     batch_val = {"data": data_val , 
                #             "target": target_val_ds}
                    ###############
